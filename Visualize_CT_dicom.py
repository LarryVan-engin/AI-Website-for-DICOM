...
#process data (DI CHUYỂN ĐỊNH NGHĨA HÀM LÊN TRÊN)
def process_data(path):
    data = pd.DataFrame([{'path': filepath} for filepath in glob(os.path.join(PATH, path))]) #Sử dụng os.path.join
...
print("tiff_data columns:", tiff_data.columns) #Kiểm tra columns
tiff_data.head(10)

#Read dicom data
print("Number of DICOM files:", len(os.listdir(os.path.join(PATH,"dicom_dir")))) #Sử dụng os.path.join
dicom_data = process_data('dicom_dir/*.dcm')
print("dicom_data columns:", dicom_data.columns) #Kiểm tra columns
dicom_data.head(10)
...
def show_dicom_images(data):
    img_data = list(data[:16].T.to_dict().values()) #số ảnh hiển thị
    f, ax = plt.subplots(4,4, figsize=(16,20))
    for i,data_row in enumerate(img_data):
