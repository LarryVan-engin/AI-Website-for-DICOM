...
files = sorted(glob.glob('train\CT_dicom\dicom_dir\*.dcm'))
images= []
target_size = (256,256)
for path in files:
    try:
        ...
        if img is not None:
            if len(img.shape) == 2: # ảnh xám
                img = cv2.cvtColor(img, cv2.COLOR_GRAY2BGR) # chuyển sang ảnh 3 kênh
            img_resized = cv2.resize(img, target_size)
            images.append(img_resized)
        else:
            print(f"Ảnh không hợp lệ (imread): {path}")
    except Exception as e:
        print(f"Lỗi khi xử lý ảnh {path}: {e}")

if images:
    images = np.array(images)
    print(f"Kích thước mảng images: {images.shape}")
else:
    print("Không có ảnh hợp lệ được đọc.")
...
for i, img in enumerate(images):
    if img is None:
        print(f"Ảnh không hợp lệ tại index: {i}")
